...
$.fn.optimizedImage = function () {
  ...
  var isBackgroundImage = $el.prop('nodeName') != 'IMG',
      ...
      fixedWidth = $el[0].hasAttribute("data-optimg-fixed")
      isFirstLoad = true;
  //instance variables
  var requestWidth,
      srcSet = [null, null, null],
      smallMediaQuery = window.matchMedia("(max-width: 749px)"),
      mediumMediaQuery = window.matchMedia("(min-width: 750px) and (max-width: 991px)"),
  ...
  function updateImageSource() {
    ...
    if (lastBreakpoint != currentBreakpoint && !isBackgroundImage && !isFirstLoad) {
  ...
  function addSelector(srcString, width) {
    ...
    return srcString.substr(0, lastIndex) + '.genecoreimg.' + width + srcString.substr(lastIndex);
  }
  ...
  function setRequestWidth() {
    ...
    for(var i = 0; i < sizes.length; i++){ //Loop the array
...
$( document ).ready(function() {
  $("[data-optimg-srcset]").each(function() {
    $(this).optimizedImage();
  });
});
...
var observer = new MutationSummary({
  ...
  queries: [{element:'*[data-optimg-srcset]'}],
...
$.widget("gene.geneTooltip", {
  ...
  _getTooltipContent : function() {
    ...
    if (this.options.content !== undefined) {
      ...
      if (this.options.title !== undefined) {
        var title = this.options.originalTerm ? this.options.originalTerm : this.options.title;
  ...
  _initializeTooltip : function() {
    ...
    this.element.tooltipster({
      trigger : 'custom',
      theme : [ 'tooltipster-genentechcore', 'tooltipster-genentechcore-customized' ],
  ...
  _modalHandler:function(instance, helper, props){
    ...
    $(instance.__Content).find(".external-link-indicator").on('click', function(e){
  ...
  _positionHandler : function(instance, helper, position) {
    ...
    if (GeneCore.utils.resizeUtil.getCurrentBreakpoint() != 'xs') {
      position.size.height = $(helper.tooltipClone).find('.tooltipster-content').outerHeight() + 10;
      if (position.side == 'top') {
        position.coord.top = helper.geo.origin.windowOffset.top - position.size.height + (this.element.is('IMG') ? $(this.element).height() - 10 : 0);
      } else {
        position.coord.top += (this.element.is('IMG') ? $(this.element).height() - 10 : 0);
      }
    }
  ...
  _checkPosition : function() {
    if (GeneCore.utils.resizeUtil.getCurrentBreakpoint() != 'xs') {
      if (this.opened && this.side == 'top') {
        ...
        if (elementTop < window.pageYOffset + (GeneCore.properties.stickyMastheadHeight || 0)) {
          this._hide();
        }
      }
    }
  ...
  _breakpointChangeHandler : function(e, breakpoint) {
    ...
    if (breakpoint == 'xs' || breakpoint == 'md') {
      if (GeneCore.properties.isMastheadStickyEnabledMobile) { // Position should be checked only if the masthead sticky option is enabled
        $(window).on('resize.tooltipster-base', _.throttle($.proxy(this._checkPosition, this), 50));
        $(window).on('scroll.tooltipster-base', _.throttle($.proxy(this._checkPosition, this), 10));
      }
    } else {
      if (GeneCore.properties.isMastheadStickyEnabledDesktop) { // Position should be checked only if the masthead sticky option is enabled
        $(window).on('resize.tooltipster-base', _.throttle($.proxy(this._checkPosition, this), 50));
        $(window).on('scroll.tooltipster-base', _.throttle($.proxy(this._checkPosition, this), 10));
      }
    }
  ...
  _hide : function() {
    ...
    $(this.element.tooltipster('elementTooltip')).find('.external-link-indicator').off('click');
  },
    ...
    try {
      $.each(obj, function(key, value){
        if (value === "" || value === null){
          ...
          value.forEach(function (el) { GeneCore.utils.generalUtils.filterNullOrEmpty(el); });
        }
      });
      return obj;
    } catch(err) {
...
$.fn.breakpoint = function () {
  //instance variables
  var beforeElement = window.getComputedStyle ? window.getComputedStyle(document.body, ':before') : false,
    currentBreakpoint = '',
    lastBreakpoint = '';

  this.init = function() {
    $(window).on('resize orientationchange load', $.proxy(this._onResizeHandler, this));
    return this;
  }

  this._onResizeHandler = function() {
    currentBreakpoint = beforeElement.getPropertyValue('content').replace(/\"/g, '');
...
$.fn.anchorsUtil = function() {
    this.init = function() {
        ...
        if (urlHash && !urlHash.startsWith('tab') && !urlHash.startsWith('accordion')) {
    ...
    function handleAnchor(event, isKeyboard) {
        ...
        if (hash && hash != '#') {
            ...
            if (includes('.cmp-safetybar .is-sticky', hash)) {
                $('.cmp-safetybar').find('.cmp-safetybar__sticky-wrapper').removeClass('is-sticky');
            }
    ...
    this.scrollToAnchor = function(targetId, event, immediate, setFocus, timeInMs) {
        ...
        if ($scrollTarget.length) {
            ...
            if (GeneCore.utils.generalUtils.getUrlHash() != targetId && !targetId.includes('#')) {
    ...
    this.scrollToYPosition = function(yPosition, immediate, timeInMs) {
        ...
        GeneCore.promises.mastheadReadyPromise.done(function() {
            var adjustedTopPosition = yPosition - (GeneCore.properties.stickyMastheadHeight || 0);
            if (immediate) {
...
GeneCore.utils.form = {
  ...
  checkForDataValue : function (data, key, value) {
    ...
    for(var index in dataArray) {
      ...
      if (keyValue && value && keyValue.toString().toLowerCase() == value.toString().toLowerCase()) {
  ...
  parsleyOptions : { excluded: "input[type=button], input[type=submit], input[type=reset], input[type=hidden], [disabled], :hidden" }
};
...
$.fn.tacticCode = function() {
    ...
    function init() {
        ...
        if(GeneCore.utils.cookieUtils.getCookie('couponCode') && GeneCore.utils.generalUtils.getUrlParam('c') === undefined) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response && response.couponCode !== null) {
                        GeneCore.utils.cookieUtils.setCookie("couponCode", response.couponCode);
                        promise.resolve();
                    } else {
                        promise.reject();
                    }
    ...
    function isSearchReferrer() {
        var searchDomains = ["google.com","yahoo.com","bing.com","duckduckgo.com","aol.com","wow.com","webcrawler.com","info.com"],
...
(function($) {
  ...
  var moc = GeneCore.utils.generalUtils.getUrlParam(mocUrlNameAlt)
