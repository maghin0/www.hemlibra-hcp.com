...
$.widget("gene.brightcovePlayer", {
   _create: function() {
    ...
    this.$transcriptLinkExpandText = this.$transcriptLink.data('transcriptLinkExpandText');
    this.$transcriptLinkCollapseText = this.$transcriptLink.data('transcriptLinkCollapseText');
...
$.widget("gene.brightcovePlaylist", {
  ...
  _initBcPlayer: function () {
    this.bcPlayer.on('loadedmetadata', $.proxy(function(){
      console.log("playing video index: " + this.bcPlayer.playlist.currentIndex());
    ...
    this.bcPlayer.on('duringplaylistchange', $.proxy(function(){
    }, this));
  },
...
$.widget("gene.table", {
  ...
  _checkTableScroll : function () {
    if (this.$tableWrapper[0].scrollWidth  > this.$tableOuterWrapper.innerWidth()) {
      this.isTableOverflowing = true;

      if (this.$tableWrapper[0].scrollWidth - this.$tableWrapper.scrollLeft() == this.$tableWrapper.outerWidth()) {
...
$.widget("gene.tabs", {
  ...
  _create: function () {
    this.$standardTabButtons = this.element.find('.cmp-tabs--standard .cmp-tabs__tab');
    this.$tabContentContainers = this.element.find('.cmp-tabs__container');

    this.$accordion = this.element.find('.cmp-tabs--accordion');
    this.$accordionTabButtons = this.element.find('.cmp-tabs__button--accordion');
  ...
  _checkTabScroll: function () {
    if (this.$tabsWrapper[0].scrollWidth > this.$tabsScrollOuter.innerWidth()) {
      ...
      if (this.$tabsWrapper[0].scrollWidth - this.$tabsWrapper.scrollLeft() == this.$tabsWrapper.outerWidth()) {
  ...
  _updateDataAnalytics: function (propertyName, propertyValue, element) {
    if (element.dataset.analytics) {
      let analytics = JSON.parse(JSON.parse("\"" + element.dataset.analytics + "\""));
...
$.widget("gene.modal", {
  ...
  _initCloseActions : function() {
    this.$confirmButton.click($.proxy(function (event) {
      this.hide();
    }, this));
  ...
  _appendAriaLevelToModal : function(){
    let headingElement = this.element.find(".cmp-modal__heading").find(':header');
    if (headingElement.length > 0) {
      let headingLevel = new RegExp('H(\\d)').exec(headingElement[0].nodeName)[1];
  ...
  function initHcpPopups() {
    ...
    if (!!hcpUrls) {
      ...
      for (var i = 0; i < newURLs.length; i++) {
        if (thisPageHref.indexOf(newURLs[i]) > -1 || thisPagePath.indexOf(newURLs[i]) > -1) {
          isHcp = true;
        }
      ...
      if (!isHcp && $hcpModal.length) {
        $(hcpLinks).on('click', function(e) {
          ...
          $hcpModal.find('.cmp-modal__button--confirm').attr({"href": href, "target": target});
          $hcpModal.modal('show');
        });
      }
    }
...
$.widget("gene.safetyBar", {
  _create: function() {
    ...
    this.isAnimationDisabled = this.element.get(0).hasAttribute("data-disable-animation");
    ...
    $(window).on('modal:show', $.proxy(function () {
    ...
    $(window).on('modal:hide', $.proxy(function () {
        this.$stickyWrapper.css('margin-right', '');
    }, this));

    this._checkPosition();

    this.$buttonBackToTop.click($.proxy(function(event){
    ...
    this.$buttonSeeMore.click($.proxy(function(event){
      if (this.isAnimationDisabled) {
        GeneCore.utils.anchorsUtil.scrollToYPosition(this.element.offset().top, true);
      } else {
    ...
    this.$buttonMinimize.click($.proxy(function(event){
      if (!this.isMinimized) {
  ...
  _setSafetyBarNormal : function () {
    this.isMinimized = false;
    this.$buttonMinimize.removeClass('cmp-safetybar__button-minimize-minimized');
...
$.widget("gene.formContainer", {
  ...
  _showErrorMessage: function (response, $widget) {
    ...
    if (response != null) {
      errorMessage += '<i>' + response.status + ' ' + response.responseText + '</i><br>';
    }
    ...
    for (index in ids) {
      ...
      if (url) {
        ...
        if (api.tokenServiceUrl == null) {
          this._postDataToUrl(postData, url, requestPromise, $widget._showErrorMessage, resultsArray);
        } else {
          $.ajax ({
            type: "GET",
            ...
            success: function (response){
              ...
              $widget._postDataToUrl(postData, url, deferreds[this.indexValue], $widget._showErrorMessage, resultsArray, requestHeadersArray);
            },
  ...
  _postDataToUrl: function (data, url, successCallback, errorCallback, resultsArray, requestHeadersArray) {
  ...
  _getFormFieldData: function(api) {
    ...
    var fieldsToSerialize = this.$form.find("input,select")
        .not(fieldsToIgnore);
...
$.widget("gene.formRules", {
   _create: function() {
    this.debugModeEnabled = GeneCore.utils.generalUtils.getUrlParam('formDebugMode') ? true : false;
    ...
    this.keepAnswersOnBackButton = this.$formContainer.data('keep-answers-on-back-button') ? true : false;
    this.applyRulesToHiddenFields = this.$formContainer.data('apply-rules-to-hidden-fields') ? true : false;
  ...
  _checkRules: function() {
    ...
    this.$rulesets.each($.proxy(function(index, ruleset) {
      this._debugMessage("============================");
      this.booleanLogicString = '';
      $(ruleset.formConditions).each($.proxy(function(index, formCondition) {
        var isConditionTrue;
  ...
  _checkFormCondition: function(formCondition) {
      ...
      case "not":
        ...
        if (!isConditionTrue && formCondition.ownValues && formCondition.ownValues.indexOf("formrule-empty") > -1) {
          if (selectedFieldValues.length == 1 && selectedFieldValues == "") {
            ...
            isConditionTrue = true;
          } else if (selectedFieldValues.includes("empty") || selectedFieldValues.length == 0) {
    ...
    if (selectedNumberValue) {this._debugMessage("selectedNumberValue: " + selectedNumberValue + " " + formCondition.logicSelect + " " + formCondition.numberValue);}
    ...
    switch(formCondition.dataConditionType) {
      ...
      case "session-data":
        ...
        if (formCondition.dataLogicSelect == "exists") {
          if (GeneCore.utils.form.getDataValueFromKey(dataToCheck,formCondition.dataName)) {
            isConditionTrue = true;
          }
          break;
        }
        ...
        if (formCondition.dataLogicSelect == "equals") {
          if (GeneCore.utils.form.checkForDataValue(dataToCheck,formCondition.dataName,formCondition.dataValue)) {
            isConditionTrue = true;
          }
          break;
        }
  ...
  _applyActions: function(formActionGroup) {
    ...
    $(formActionGroup.hideShowActions).each($.proxy(function(index, hideShowAction) {
      ...
      if (hideShowAction.actionType == "show") {
        $(hideShowAction.actionTargets).each($.proxy(function(index, actionTarget) {
      ...
      if (hideShowAction.actionType == "set-input-value") {
        $(hideShowAction.actionTargets).each($.proxy(function(index, actionTarget) {
          ...
          $(fieldArray).each($.proxy(function(index, actionTarget) {
            $target = $(actionTarget);
  ...
  _clearInputsIfBackButton: function() {
    $(window).bind("pageshow", $.proxy(function(index, el) {
...
$.widget("gene.paginationContainer", {
  ...
  _create: function() {
    ...
    this.$submitButton.click($.proxy(function(e) {
...
$.widget("gene.masthead", {
  _create: function() {
    ...
    this.$mobileLinks = this.element.find('.cmp-masthead__utility-nav-mobile-item, .cmp-masthead__primary-nav__item:not(.cmp-masthead__primary-nav__item--dropdown)');
...
$.widget("gene.stickyNav", {
  _create: function() {
    ...
    if (((currentBreakpoint == 'xs' || currentBreakpoint == 'md') && this.stickyEnabledMobile)
  ...
  _breakpointChangeHandler: function(e, breakpoint) {
    ...
    if (((breakpoint == 'xs' || breakpoint == 'md') && (this.stickyOptionMobile == 'all-sticky' || this.stickyOptionMobile == 'hide-links') && this.stickyEnabledMobile)
      ...
      if ((breakpoint == 'xs' || breakpoint == 'md') && this.stickyOptionMobile == 'hide-links' && this.stickyEnabledMobile) {
        ...
        $(window).on('scroll-direction-change', _.throttle(function(e, direction) {
          if (direction == 'up') {
...
$.widget("gene.references", {
  ...
  _initReference: function ($reference) {
    ...
    var refNumber = $reference.is('img') ? $reference.data('reference-number') : refText;
    var refElements = {};
...
$.widget("gene.searchResults", {
    ...
    _goToPage: function(offset) {
        ...
        $.ajax({
            url: window.location.pathname.substr(0, window.location.pathname.lastIndexOf(".")) + '.site.search.html?isAjax=true&offset='+offset+'&q=' + query + "&r=" + resultCount,
            type: 'GET',
            dataType:'json',
    ...
    _populateSearchInput: function() {
      ...
      if (query !== null) {
        ...
        this.element.find('.cmp-searchinput__input').val(decodeURIComponent(query.replace(new RegExp("\\+","g"),' ')));
      }
    },
   ...
   _create: function() {
    ...
    _removeLink:function(){
        this.$resources.each(function() {
            ...
            var $c = $element
                .clone()
                .css({display: 'block', width: $element.width(), height: 'auto', visibility: 'hidden'})
                .appendTo('body');
        ...
        this.filter.bind('filterChange', function(event){
            if (selected === 'Show All') {
                ...
                _this.$resources.each(function(index){
                    if ($(this).not('[data-resource-tags]').length || $(this).data('resourceTags').search(selected) === -1) {
                        $(this).hide();
    ...
    _countSelected: function(setText) {
        ...
        if (setText) {
            var selectString = selectCount === 1 ? selectCount + " file selected" : selectCount + " files selected";
...
$.widget("gene.searchInput", {
    ...
    _create: function() {
        ...
        if(this.element.find('.cmp-searchinput__container').data('require-two-terms')){
...
$.widget("gene.datalookupresults", {
    ...
    _getLocation: function () {
        return window.location.pathname.replace(/\.(html)($|\?)/, '.data.search.' + this.cmpId + '.json$2');
    },
